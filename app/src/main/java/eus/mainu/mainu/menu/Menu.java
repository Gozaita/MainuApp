...
public class Menu implements Serializable {
	...
	public ArrayList<Imagen> getImagenes() {
		...
		for (i = 0; i < primeros.size(); i++) {
			if (!primeros.get(i).getImagenes().isEmpty()) {
				imagenes.add(primeros.get(i).getImagenes().get(0));
			} else {
		...
		//Cogemos las imagenes oficiales de los platos
		for (i = 0; i < segundos.size(); i++) {
			if (!segundos.get(i).getImagenes().isEmpty()) {
				imagenes.add(segundos.get(i).getImagenes().get(0));
			} else {
				imagenes.add(new Imagen(999, "noImagen", new Usuario(999, "mainu", "", 0)));
			}
		}
		...
		//Cogemos las imagenes oficiales de los platos
		for (i = 0; i < postres.size(); i++) {
			if (!postres.get(i).getImagenes().isEmpty()) {
				imagenes.add(postres.get(i).getImagenes().get(0));
			} else {
				imagenes.add(new Imagen(999, "noImagen", new Usuario(999, "mainu", "", 0)));
			}
		}
